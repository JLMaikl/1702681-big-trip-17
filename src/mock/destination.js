...
const generateDescription = () => {
  const sentencesCount = getRandomInteger(
    NumberOfSentences.MIN,
    NumberOfSentences.MAX
  );
  const descriptionSentences = DESCRIPTION.split('. ');
  const description = `${descriptionSentences
...
const generatePictures = () => {
  const picturesCount = getRandomInteger(
    NumberOfPictures.MIN,
    NumberOfPictures.MAX
  );
  const pictures = [];
...
export { generateDestination, DESTINATIONS_NAMES };
