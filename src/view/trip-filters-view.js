...
const createFilterItemTemplate = (filter, isChecked) => {
  const { name, coint } = filter;

  return (
    `<div class='trip-filters__filter'>
        <input
          id='filter-${name}' class='trip-filters__filter-input  visually-hidden' type='radio' name='trip-filter'
          value='${name}'
          ${isChecked ? 'checked' : ''}
          ${coint === 0 ? 'disabled' : ''}
        >
        <label
          class='trip-filters__filter-label' for='filter-${name}'
        >${name}</label>
      </div>`
  );
};
